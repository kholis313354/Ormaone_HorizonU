    ...
    <div class="d-flex align-items-center justify-content-between gap-3">
        ...
        <div class="header-left">
            ...
            <div class="d-none d-md-inline-block">
                <div class="modern-search">
                    ...
                    <input type="text" placeholder="Search fakultas" aria-label="Search" id="modernSearchInput" />
        ...
        <div class="header-right">
            ...
            <div class="dropdown text-light ">
                <a class="user-dropdown-btn text-light" id="navbarDropdownUser" href="#" role="button"
                    data-bs-toggle="dropdown" aria-expanded="false" data-bs-auto-close="true">
                    <div class="user-avatar">
                        ...
                        if (!empty($profilePhoto)) {
                            $profilePath = FCPATH . 'uploads/profile/' . $profilePhoto;
                            if (file_exists($profilePath)) {
                                $profileImageUrl = base_url('uploads/profile/' . $profilePhoto);
                                $hasProfileImage = true;
                            }
                        }

                        if ($hasProfileImage): ?>
                            <img src="<?= esc($profileImageUrl) ?>" alt="Profile"
                                onerror="this.style.display='none'; this.parentElement.querySelector('i').style.display='flex';"
                                onload="this.parentElement.querySelector('i').style.display='none';">
                            <i class="fas fa-user" style="display: none;"></i>
                        <?php else: ?>
                            <i class="fas fa-user"></i>
                        <?php endif; ?>
                    </div>
                    <span class="user-name d-none d-sm-inline text-light"><?= session()->get('name') ?? 'User' ?></span>
    ...
    <section class="section">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="<?= url_to('fakultas.store') ?>" method="POST">
                    <?= csrf_field() ?>
                    <div class="mb-3">
                        <label for="nama_fakultas" class="form-label">Nama Fakultas</label>
                        <input type="text" class="form-control" id="nama_fakultas" name="nama_fakultas"
                            placeholder="Nama Fakultas" required value="<?= old('nama_fakultas') ?>">
                        <?php if (session()->has('errors.nama_fakultas')): ?>
                            <div class="text-danger small"><?= session('errors.nama_fakultas') ?></div>
                        <?php endif; ?>
                    </div>
                    <button type="submit" class="btn btn-primary">Simpan</button>
                    <a href="<?= url_to('fakultas.index') ?>" class="btn btn-secondary">Kembali</a>
                </form>
            </div>
        </div>
    </section>
