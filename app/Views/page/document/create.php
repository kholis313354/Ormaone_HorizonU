    ...
    <section class="section">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="<?= url_to('document.store') ?>" method="POST" enctype="multipart/form-data">
                    <?= csrf_field() ?>
                    <div class="mb-3">
                        <label for="judul" class="form-label">Judul Document <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="judul" name="judul"
                            placeholder="Masukkan judul document" required>
                    </div>
                    <div class="mb-3">
                        <label for="kategori" class="form-label">Kategori <span class="text-danger">*</span></label>
                        <select name="kategori" class="form-select" required <?= (isset($selectedKategori) && !empty($selectedKategori)) ? 'disabled' : '' ?>>
                            <option value="">--Pilih Kategori--</option>
                            <option value="PB" <?= (isset($selectedKategori) && $selectedKategori == 'PB') ? 'selected' : '' ?>>PB</option>
                            <option value="CA" <?= (isset($selectedKategori) && $selectedKategori == 'CA') ? 'selected' : '' ?>>CA</option>
                            <option value="PRS" <?= (isset($selectedKategori) && $selectedKategori == 'PRS') ? 'selected' : '' ?>>PRS</option>
                            <option value="POA" <?= (isset($selectedKategori) && $selectedKategori == 'POA') ? 'selected' : '' ?>>POA</option>
                            <option value="KPI" <?= (isset($selectedKategori) && $selectedKategori == 'KPI') ? 'selected' : '' ?>>KPI</option>
                            <option value="AD/ART" <?= (isset($selectedKategori) && $selectedKategori == 'AD/ART') ? 'selected' : '' ?>>AD/ART</option>
                        </select>
                        <?php if (isset($selectedKategori) && !empty($selectedKategori)): ?>
                            <input type="hidden" name="kategori" value="<?= esc($selectedKategori) ?>">
                            <small class="form-text text-muted">Kategori sudah terisi otomatis berdasarkan menu yang
                                dipilih</small>
                        <?php endif; ?>
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3"
                            placeholder="Masukkan deskripsi document (opsional)"></textarea>
                    </div>
                    <div class="mb-3">
                        <label for="tahun" class="form-label">Tahun <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" id="tahun" name="tahun"
                            placeholder="Tahun (contoh: 2025)" min="2000" max="2100" value="<?= date('Y') ?>" readonly
                            required>
                        <small class="form-text text-muted">Tahun otomatis terisi dengan tahun saat ini</small>
                    </div>
                    <div class="mb-3">
                    ...
                    <a href="<?= url_to('document.index') . ($selectedKategori ? '?kategori=' . urlencode($selectedKategori) : '') ?>"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (tahunInput) {
            ...
            // Update setiap detik (optional, untuk realtime)
            setInterval(function () {
                const newYear = new Date().getFullYear();
                if (tahunInput.value != newYear) {
