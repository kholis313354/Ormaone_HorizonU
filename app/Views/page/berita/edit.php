        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <!-- PERUBAHAN UTAMA: Tambahkan enctype="multipart/form-data" -->
                    <form action="<?= base_url('page/berita//update/' . $berita['id']) ?>" method="post" enctype="multipart/form-data">
                        <?= csrf_field() ?>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="nama_kegiatan" name="nama_kegiatan"
                                    value="<?= old('nama_kegiatan', $berita['nama_kegiatan']) ?>" required>
                            </div>
                            <div class="col-md-6">
                                <label for="kategori" class="form-label">Kategori</label>
                                <select class="form-select" id="kategori" name="kategori" required>
                                    <option value="">Pilih Kategori</option>
                                    <option value="blogger" <?= old('kategori', $berita['kategori']) == 'blogger' ? 'selected' : '' ?>>Blogger</option>
                                    <option value="podcast" <?= old('kategori', $berita['kategori']) == 'podcast' ? 'selected' : '' ?>>Podcast</option>
                                </select>
                            </div>
                        </div>

                        <!-- Field Link (hanya muncul jika kategori podcast) -->
                        <div class="mb-3" id="link-field" style="display: <?= old('kategori', $berita['kategori']) == 'podcast' ? 'block' : 'none' ?>;">
                            <label for="link" class="form-label">Link Video YouTube <span class="text-danger">*</span></label>
                            <input type="url" class="form-control <?= session('errors.link') ? 'is-invalid' : '' ?>"
                                id="link" name="link" value="<?= old('link', $berita['link'] ?? '') ?>" 
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                ...
                                kategoriSelect.addEventListener('change', toggleLinkField);
                            });
                        </script>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="fakultas_id" name="fakultas_id" required>
                                    ...
                                    <?php foreach ($fakultas as $f): ?>
                                        <option value="<?= $f['id'] ?>"
                                            <?= ($f['id'] == old('fakultas_id', $berita['fakultas_id'])) ? 'selected' : '' ?>>
                                            <?= $f['nama_fakultas'] ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <?php if (!empty($berita['gambar'])) : ?>
                                <div class="mb-2">
                                    ...
                                    <div class="form-text mt-2">
                                        <a href="<?= base_url('uploads/berita/' . $berita['gambar']) ?>"
                            ...
                            <div class="form-text">Biarkan kosong jika tidak ingin mengubah gambar (Format: JPG, PNG, maks 2MB)</div>
                            <?php if (session('errors.gambar')) : ?>
                                <div class="text-danger small"><?= session('errors.gambar') ?></div>
                            <?php endif; ?>
                        </div>

                        <div class="mb-3">
                            <label for="deskripsi1" class="form-label">Deskripsi Utama</label>
                            <textarea class="form-control" id="deskripsi1" name="deskripsi1" rows="3" required><?= old('deskripsi1', $berita['deskripsi1']) ?></textarea>
                        </div>

                        <div class="mb-3">
                            <label for="deskripsi2" class="form-label">Deskripsi Tambahan (Opsional)</label>
                            <textarea class="form-control" id="deskripsi2" name="deskripsi2" rows="3"><?= old('deskripsi2', $berita['deskripsi2']) ?></textarea>
                        </div>

                        <div class="mb-3">
                            <label for="deskripsi3" class="form-label">Deskripsi Tambahan (Opsional)</label>
                            <textarea class="form-control" id="deskripsi3" name="deskripsi3" rows="3"><?= old('deskripsi3', $berita['deskripsi3']) ?></textarea>
                        </div>

                        <div class="d-flex justify-content-end">
                            <a href="<?= url_to('berita.index') ?>" class="btn btn-secondary">Kembali</a>
