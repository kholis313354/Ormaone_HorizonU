...
<style>
    ...
    /* Google Forms-like styling - Same as create.php */
...
<div class="form-builder-container">
    ...
    <div class="form-builder-content">
        ...
        <?php if (!empty($form['encrypted_link'])): ?>
            <div class="alert alert-info mb-3">
                <i class="bi bi-info-circle"></i>
                <strong>Form sudah dipublish!</strong><br>
                Link: <a href="<?= base_url('form/public/' . $form['encrypted_link']) ?>"
                    target="_blank"><?= base_url('form/public/' . $form['encrypted_link']) ?></a><br>
                <div class="mt-2">
                    QR Code:
                    <?php
                    $hasQRCode = !empty($form['qr_code_path']);
                    $qrCodeFullPath = $hasQRCode ? FCPATH . $form['qr_code_path'] : null;
                    $qrCodeExists = $hasQRCode && $qrCodeFullPath && file_exists($qrCodeFullPath);

                    if ($qrCodeExists): ?>
                        <div class="mt-2">
                            <img src="<?= base_url($form['qr_code_path']) ?>" alt="QR Code"
                                style="max-width: 200px; border: 1px solid #ddd; padding: 10px; background: #fff; display: block;">
                            <a href="<?= base_url($form['qr_code_path']) ?>" target="_blank" class="btn btn-sm btn-primary mt-2"
                                download>
                    ...
                    <?php else: ?>
                        <div class="mt-2">
                            <p class="text-muted mb-2">QR Code belum digenerate.</p>
                            <a href="<?= url_to('form.qrcode', $form['id']) ?>" class="btn btn-sm btn-success">
        ...
        <form id="formBuilderForm" action="<?= url_to('form.update', $form['id']) ?>" method="POST"
            ...
            <div class="form-field-card"
                ...
                <div style="position: relative; padding: 10px;">
                    ...
                    ?>
                    <img id="headerImagePreview" src="<?= base_url($headerImage) ?>"
            ...
            <div class="form-title-section">
                <div contenteditable="true" id="formTitle" class="form-title-input" data-placeholder="Untitled form"
                    style="min-height: 40px;" onfocus="showFormattingToolbar('title')"
                    onblur="hideFormattingToolbar('title')" oninput="updateFormTitle()"><?= $form['title'] ?></div>
                <input type="hidden" name="title" id="formTitleHidden" value="<?= $form['title'] ?>" required>
                ...
                <div contenteditable="true" id="formDescription" class="form-description-input"
                    ...
                    onfocus="showFormattingToolbar('description')" onblur="hideFormattingToolbar('description')"
                    oninput="updateFormDescription()"><?= $form['description'] ?? '' ?></div>
                <input type="hidden" name="description" id="formDescriptionHidden"
                    value="<?= $form['description'] ?? '' ?>">

                <div class="formatting-toolbar hidden" id="titleFormattingToolbar">
                    <button type="button" class="format-btn" onclick="formatText('formTitle', 'bold')" title="Bold">
                        <i class="bi bi-type-bold"></i>
                    </button>
                    <button type="button" class="format-btn" onclick="formatText('formTitle', 'italic')" title="Italic">
                        <i class="bi bi-type-italic"></i>
                    </button>
                    <button type="button" class="format-btn" onclick="formatText('formTitle', 'underline')"
                        title="Underline">
                        <i class="bi bi-type-underline"></i>
                    </button>
                    <div class="format-divider"></div>
                    <button type="button" class="format-btn" onclick="formatText('formTitle', 'insertLink')"
                        title="Insert link">
                        <i class="bi bi-link-45deg"></i>
                    </button>
                    <button type="button" class="format-btn" onclick="formatText('formTitle', 'strikeThrough')"
                        title="Strikethrough">
                ...
                <div class="formatting-toolbar hidden" id="descriptionFormattingToolbar">
                    <button type="button" class="format-btn" onclick="formatText('formDescription', 'bold')"
                        title="Bold">
                        <i class="bi bi-type-bold"></i>
                    </button>
                    <button type="button" class="format-btn" onclick="formatText('formDescription', 'italic')"
                        title="Italic">
                        <i class="bi bi-type-italic"></i>
                    </button>
                    <button type="button" class="format-btn" onclick="formatText('formDescription', 'underline')"
                        title="Underline">
                        <i class="bi bi-type-underline"></i>
                    </button>
                    <div class="format-divider"></div>
                    <button type="button" class="format-btn"
                        onclick="formatText('formDescription', 'insertUnorderedList')" title="Bullet list">
                        <i class="bi bi-list-ul"></i>
                    </button>
                    <button type="button" class="format-btn"
                        onclick="formatText('formDescription', 'insertOrderedList')" title="Numbered list">
                        <i class="bi bi-list-ol"></i>
                    </button>
                    <div class="format-divider"></div>
                    <button type="button" class="format-btn" onclick="formatText('formDescription', 'insertLink')"
                        title="Insert link">
                        <i class="bi bi-link-45deg"></i>
                    </button>
                    <button type="button" class="format-btn" onclick="formatText('formDescription', 'removeFormat')"
                ...
                <div class="form-meta-row">
                    <div class="form-meta-item">
                        ...
                        <select name="form_type" id="form_type" required>
                            ...
                            <option value="absensi" <?= $form['form_type'] == 'absensi' ? 'selected' : '' ?>>Absensi
                            </option>
                            <option value="quiz" <?= $form['form_type'] == 'quiz' ? 'selected' : '' ?>>Quiz</option>
                            <option value="survey" <?= $form['form_type'] == 'survey' ? 'selected' : '' ?>>Survey</option>
                            <option value="pendaftaran" <?= $form['form_type'] == 'pendaftaran' ? 'selected' : '' ?>>
                                Pendaftaran</option>
                            <option value="umum" <?= $form['form_type'] == 'umum' ? 'selected' : '' ?>>Umum</option>
                        </select>
                    </div>
                    <div class="form-meta-item">
                        ...
                        <select name="organisasi_id" id="organisasi_id">
                            ...
                            <?php if (isset($organisasis)): ?>
                                <?php foreach ($organisasis as $organisasi): ?>
                                    <option value="<?= $organisasi['id'] ?>" <?= ($form['organisasi_id'] == $organisasi['id']) ? 'selected' : '' ?>>
                                        <?= esc($organisasi['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            <?php endif; ?>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Initialize form title and description
        updateFormTitle();
        updateFormDescription();

        // Set placeholder for contenteditable elements
        ...
        <?php if (isset($fields) && !empty($fields)): ?>
            <?php foreach ($fields as $index => $field): ?>
                fieldCounter++;
                const existingField_<?= $index ?> = {
                    id: 'field_' + fieldCounter,
                    type: '<?= esc($field['field_type']) ?>',
                    label: <?= json_encode($field['label']) ?>,
                    placeholder: <?= json_encode($field['placeholder'] ?? '') ?>,
                    description: <?= json_encode($field['description'] ?? '') ?>,
                    isRequired: <?= $field['is_required'] ? 'true' : 'false' ?>,
                    options: <?= !empty($field['options']) ? json_encode(json_decode($field['options'], true)) : '[]' ?>,
                    order: <?= $index ?>
                };
                fields.push(existingField_<?= $index ?>);
                renderField(existingField_<?= $index ?>);
            <?php endforeach; ?>
            updateFieldsData();
            updateEmptyState();
        <?php endif; ?>
