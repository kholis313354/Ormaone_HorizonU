    ...
    <section class="section">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <?php if (empty($pemilihanCalons)): ?>
                    <div class="alert alert-warning">
                        <i class="bi bi-exclamation-triangle"></i> Tidak ada data pemilihan yang sudah selesai. Hanya
                        pemilihan dengan status "selesai" yang dapat dibuat menjadi kepengurusan.
                    </div>
                <?php endif; ?>

                <form action="<?= url_to('organisasi.kepengurusan.store') ?>" method="POST"
                    enctype="multipart/form-data">
                    <?= csrf_field() ?>
                    <div class="mb-3">
                        <label for="pemilihan_calon_id" class="form-label">Pilih Calon dari Pemilihan yang
                            Selesai</label>
                        <select name="pemilihan_calon_id" id="pemilihan_calon_id" class="form-select" required>
                            <option value="">--Pilih Calon--</option>
                            <?php foreach ($pemilihanCalons as $calon): ?>
                                <option value="<?= $calon['id'] ?>" data-organisasi-id="<?= $calon['organisasi_id'] ?>"
                                    data-anggota-1-id="<?= $calon['anggota_1_id'] ?>"
                                    data-anggota-2-id="<?= $calon['anggota_2_id'] ?? '' ?>"
                                    data-periode="<?= $calon['pemilihan_periode'] ?>">
                                    <?= $calon['organisasi_name'] ?> -
                                    <?= $calon['anggota_1_name'] ?>    <?= !empty($calon['anggota_2_name']) ? ' & ' . $calon['anggota_2_name'] : '' ?>
                                    (<?= $calon['pemilihan_periode'] ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <small class="form-text text-muted">Hanya menampilkan calon dari pemilihan yang statusnya
                            "selesai"</small>
                    </div>
                    <div class="mb-3">
                        <label for="organisasi_id" class="form-label">Organisasi</label>
                        <select name="organisasi_id" id="organisasi_id" class="form-select" required readonly>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="anggota_id" id="anggota_id" class="form-select" required readonly>
                    ...
                    <button type="submit" class="btn btn-primary" <?= empty($pemilihanCalons) ? 'disabled' : '' ?>>Simpan</button>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const pemilihanCalonSelect = document.getElementById('pemilihan_calon_id');
        ...
        pemilihanCalonSelect.addEventListener('change', function () {
            const selectedOption = this.options[this.selectedIndex];

            if (selectedOption.value) {
                const organisasiId = selectedOption.getAttribute('data-organisasi-id');
                const anggota1Id = selectedOption.getAttribute('data-anggota-1-id');
                const anggota2Id = selectedOption.getAttribute('data-anggota-2-id');
                const periode = selectedOption.getAttribute('data-periode');

                // Set organisasi
                organisasiSelect.value = organisasiId;

                // Set periode
                periodeInput.value = periode;

                // Clear and populate anggota select
                ...
                if (anggota1Id) {
                    fetch(`<?= base_url('organisasi/kepengurusan/anggota') ?>/${organisasiId}`)
                        ...
                        .then(data => {
                            // Add anggota 1
                            const anggota1 = data.find(a => a.id == anggota1Id);
                            if (anggota1) {
                                const option1 = document.createElement('option');
                                option1.value = anggota1.id;
                                option1.textContent = `${anggota1.nim} - ${anggota1.name}`;
                                option1.selected = true;
                                anggotaSelect.appendChild(option1);
                            }

                            // Add anggota 2 if exists
                            if (anggota2Id) {
                                const anggota2 = data.find(a => a.id == anggota2Id);
                                if (anggota2) {
                                    const option2 = document.createElement('option');
                                    option2.value = anggota2.id;
                                    option2.textContent = `${anggota2.nim} - ${anggota2.name}`;
                                    anggotaSelect.appendChild(option2);
                                }
                            }
                        })
