        ...
        <section class="section">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="<?= url_to('organisasi.kepengurusan.update', $data['id']) ?>" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="jabatan" name="jabatan" placeholder="Jabatan" required value="<?= old('jabatan', $data['jabatan']) ?>">
                        </div>
                        <div class="mb-3">
                            <label for="periode" class="form-label">Periode</label>
                            <input type="text" class="form-control" id="periode" name="periode" placeholder="Periode" required value="<?= old('periode', $data['periode']) ?>">
                        </div>
...
<?= $this->section('scripts') ?>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setTimeout(() => {
                const selectedOrganisasiId = '<?= $data['organisasi_id'] ?>';
                organisasiSelect.value = selectedOrganisasiId;
                organisasiSelect.dispatchEvent(new Event('change'));
                setTimeout(() => {
                    const selectedAnggotaId = '<?= $data['anggota_id'] ?>';
                    anggotaSelect.value = selectedAnggotaId;
